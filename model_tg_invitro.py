...
random.seed(1)
tg_rat_invitro_mean_prop=batchFS("./mean_TG_","./tg_invitro_label.csv",0,108)
tg_invitro_mean_res=cptRes(tg_rat_invitro_mean_prop,label)
tg_invitro_mean_res=pd.DataFrame(tg_invitro_mean_res)
tg_invitro_mean_res.to_csv("./tg_invitro_mean_res.csv")

data=pd.read_csv(open("./mean_TG_8.csv"))
data=data.values
data=data[:,1:len(data[1,:])]
label=pd.read_csv(open("./tg_invitro_label.csv"))
label=label.values
label=label[:,1]
